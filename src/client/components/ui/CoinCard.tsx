    ...
    >
      ...
      <div className="relative h-16 bg-gradient-to-r from-blue-400 to-blue-600">
        ...
        <div className={`absolute top-4 right-4 px-3 py-1 rounded-full ${
          coin.bitSlow ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-500"
        } text-xs font-bold`}>
      ...
      {/* Card Body */}
      <div 
        className="p-6 pt-10 cursor-pointer"
        onClick={() => onClick?.(coin)}
      >
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-xl font-bold text-gray-800">Coin #{coin.bitSlow || coin.coin_id}</h3>
            <p className="text-blue-500 font-medium">${coin.value.toFixed(2)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div className="flex items-center text-gray-600">
            ...
            <span className="ml-2">{coin.bit1}, {coin.bit2}, {coin.bit3}</span>
          </div>
      ...
      {/* Card Footer */}
      {coin.client_id === null && buyButtonEnabled && (
