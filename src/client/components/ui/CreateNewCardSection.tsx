...
import { PlusCircleIcon, CoinsIcon, SparklesIcon, TrendingUpIcon, AlertCircleIcon, LockIcon, TimerIcon, BarChartIcon } from "lucide-react";
      ...
      <section className="bg-gray-100 p-6 rounded-2xl items-center justify-center shadow-md flex flex-col h-full relative overflow-hidden">
        ...
        <div className="flex flex-col gap-3 mb-8 relative z-10 text-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-gray-600">Mining Complete</h2>
          </div>
          ...
          <p className="text-gray-600 text-sm mt-2 max-w-sm">
            Our miners have discovered all available coins in this round. 
            Check back soon for the next mining opportunity!
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 mb-8 relative z-10">
        ...
        <div className="grid grid-cols-2 gap-2 my-3">
          ...
          <div className="bg-blue-50 p-3 rounded-lg">
            <p className="text-xs text-blue-700 font-medium">AVG. MINING TIME</p>
            <div className="flex items-center">
        ...
        <div className={`flex items-center mt-2 ${remainingItem <= 5 ? 'bg-red-50' : 'bg-yellow-50'} p-3 rounded-lg`}>
          <CoinsIcon className={`w-4 h-4 ${remainingItem <= 5 ? 'text-red-500' : 'text-yellow-600'} mr-2`} />
          <p className={`${remainingItem <= 5 ? 'text-red-500' : 'text-yellow-700'} font-semibold text-sm`}>
            {remainingItem <= 5 ? (
              <>Critical alert – only <span className="underline font-bold">{remainingItem} coins</span> left to be discovered!</>
            ) : (
