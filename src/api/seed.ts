...
function seedCoins(
  ...
  `);
	...
	// Calculate how many free coins to create based on percentage
	const freeCoinCount = Math.floor(count * (freePercentage / 100));
	const ownedCoinCount = count - freeCoinCount;

	console.log(
		`Creating ${ownedCoinCount} owned coins and ${freeCoinCount} free coins`,
	);

	// Create owned coins first
	for (let i = 0; i < ownedCoinCount; i++) {
		...
		console.log(`Owned: ${clientId} -> ${coinId}`);
		coinIds.push(coinId);
	}

	// Create free coins (unowned)
	for (let i = 0; i < freeCoinCount; i++) {
		let bit1: number, bit2: number, bit3: number;
		let combo;
		do {
			[bit1, bit2, bit3] = generateDistinctRandomValues(3, 1, 10);
		...
		// Insert with null client_id for free coins
		const info = insert.run(null, bit1, bit2, bit3, value);
		const coinId = Number(info.lastInsertRowid);
		console.log(`Free: null -> ${coinId}`);
		coinIds.push(coinId);
	}
