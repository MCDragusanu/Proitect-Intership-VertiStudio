...
export class SQLLiteUserRepository implements UserRepository {
    ...
    async updateCredentials(credentials: UserCredentials): Promise<boolean> {
        ...
        try {
            ...
            console.log(`[Credentials] Updated user with email: ${credentials.user_email} - Success: ${success}`);
        }
    }
    ...
    async getUserCredentialsByEmail(userEmail: string): Promise<UserCredentials | null> {
        let credentials: UserCredentials | null = null;
        try {
            credentials = await this.userCredentialsDao.getCredentialsByEmail(userEmail);
            return credentials;
        } catch (err: any) {
    ...
    async getUserCredentialsByToken(refreshToken: string): Promise<UserCredentials | null> {
        let credentials: UserCredentials | null = null;
        try {
    ...
    async getUserCredentialsByUid(userUid: string): Promise<UserCredentials | null> {
        let credentials: UserCredentials | null = null;
        try {
