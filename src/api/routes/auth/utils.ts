...
export const validateEmail = (email: string): boolean => {
	const emailValid = /^[A-Za-z._0-9]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(
		email,
	);
	return emailValid;
};
...
export const validateUserProfile = (
	...
	if (!isFirstNameValid)
		fieldErrors.push("First Name must only contain letters.");
	if (!isLastNameValid)
		fieldErrors.push("Last Name must only contain letters.");
